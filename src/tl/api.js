...
class CastError extends Error {
  constructor(
    ...
    const message =
      `Found wrong type for ${objectName}. Expected ${expected}, but recieved \
${actual}. If you think this is a mistake please report it.`;
...
function createClasses(_classesType, params) {
  const classes = {};
        ...
        for (const arg in argsConfig) {
          if (argsConfig.hasOwnProperty(arg)) {
            if (arg === "flags" || argsConfig[arg].isFlag) {
              continue; // We don't care about flags.
            }
          ...
          if (argsConfig.hasOwnProperty(arg)) {
            ...
            if (argsConfig[arg].isVector) {
              ...
              if (
                ...
                const boxed = argsConfig[arg].type.charAt(
                  argsConfig[arg].type.indexOf(".") + 1,
                );
                const isBoxed = boxed === boxed.toUpperCase();
                if (!isBoxed) {
                  buffers.shift();
                }
              }
...
export const Api = buildApiTlSchema();
